                ...
                pager.pagination({
                    onSelectPage: function (pageNum, pageSize) {
                        opts.onBeforeLoad = function (param) {
                            ...
                            param.page = pageNum;
                            param.rows = pageSize;
                            dg.datagrid('getPager').pagination('refresh', { pageNumber: pageNum, pageSize: pageSize });
                        };
                        dg.datagrid('load');
                    }
                });
        ...
        function initCss() {
            if (!$('#datagrid-filter-style').length) {
                $('head').append(
					...
					'a.datagrid-filter-btn{margin-left:0px !important;display:inline-block;width:22px;height:22px;vertical-align:top;cursor:pointer;opacity:0.6;filter:alpha(opacity=60);}' +
					'a:hover.datagrid-filter-btn{opacity:1;filter:alpha(opacity=100);text-decoration:none !important;}' +
					'</style>'
				);
            }
        }
            ...
            menu.menu({
                ...
                onClick: function (item) {
                    ...
                    if (item.name == "nofilter") {
                        if ($(input[0]).attr("class").indexOf("numberbox") > -1)
                            $(input[0]).numberbox('setValue', '');
                        else
                            $(input[0]).val("");
                    }
